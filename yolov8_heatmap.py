...
def get_params():
    params = {
        'weight': 'E:/NetWork/Classic_model/yolov8/runs/detect/train-PCSA 2/weights/best.pt',
        'cfg': 'E:/NetWork/Classic_model/yolov8/ultralytics/cfg/models/v8/yolov8s.yaml',
        'device': 'cuda:0',
...
def batch_process_folder(input_folder, output_folder):
    ...
    for image_file in image_files:
        ...
        save_path = os.path.join(output_folder, image_file.replace('.', '_result.'))  # 修改保存路径
...
def batch_process_images(images, output_folder):
    for image_file in images:
        try:
            ...
            print("Please run the remaining images manually in CMD.")
            break

if __name__ == '__main__':
    model = yolov8_heatmap(**get_params())

    input_folder = r'E:\\NetWork\\Classic_model\\yolov8\\photo'  # 或者使用双反斜杠：'E:\\NetWork\\Classic_model\\yolov8\\photo'
    output_folder_base = get_unique_folder_name()
    ...
    for i in range(0, len(image_files), batch_size):
        ...
        output_folder = os.path.join(output_folder_base, f'batch_{i // batch_size + 1}')
        os.makedirs(output_folder, exist_ok=True)

        # 处理一组图像
        batch_process_images(batch_images, output_folder)
